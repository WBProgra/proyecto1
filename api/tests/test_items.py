...
from api.models import Item, Usuario
...
def test_user(db):
    ...
    return Usuario.objects.create_user(email="testuser@example.com", password="password123")
...
def auth_headers(api_client, test_user):
    ...
    response = api_client.post("/graphql/", json.dumps(data), content_type="application/json")

    # Verificar que la obtención del token fue exitosa
    assert response.status_code == 200
    content = response.json()
    assert "errors" not in content, f"Error al obtener el token: {content.get('errors')}"
